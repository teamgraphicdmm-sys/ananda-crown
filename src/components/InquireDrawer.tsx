...
import { PROJECT_DETAILS, InventoryUnit } from "@/data/projectData";
...
export default function InquireDrawer({
  isOpen,
  onClose,
  selectedUnit,
}: InquireDrawerProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    request: selectedUnit
      ? `Interest in Unit ${selectedUnit.fraction} (${selectedUnit.typology}) on ${selectedUnit.floorLabel}`
      : "",
  });
  ...
  return (
    <div className="fixed inset-0 z-[99999] w-full h-full bg-[#0E0703] text-white flex flex-col justify-between px-6 sm:px-12 md:px-16 py-8 md:py-12 overflow-y-auto select-none animate-in fade-in duration-300">
      {/* Top Close Button */}
      <div className="w-full flex justify-end items-center mb-6">
        <button
          onClick={onClose}
          className="text-xs sm:text-[13px] tracking-[0.18em] uppercase font-medium text-white hover:text-[#A27B58] transition-colors cursor-pointer"
        >
          CLOSE
        </button>
      </div>
