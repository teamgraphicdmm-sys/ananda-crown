...
const allSteps = [
  ...
  2146, 2152, 2190, 2345, 2349, 2353, 2389, 2464, 2470, 2673, 2714, 2780
];

for (const s of allSteps) {
  const tc = getStepData(s).tool_calls[0].args;
  if (!html.includes(tc.TargetContent)) {
    console.log(`Step ${s} FAILED! Target length: ${tc.TargetContent.length}`);
  } else {
    html = html.replace(tc.TargetContent, tc.ReplacementContent);
  }
}
console.log('Bytes:', Buffer.byteLength(html, 'utf8'), 'Lines:', html.split('\n').length);
