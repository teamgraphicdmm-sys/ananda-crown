...
rl.on('line', (line) => {
  try {
    ...
    if (d.step_index >= 770 && d.step_index <= 836) {
      if (d.tool_calls) {
        console.log(`Step ${d.step_index}: ${d.tool_calls[0].name} -> ${JSON.stringify(d.tool_calls[0].args).slice(0, 150)}`);
      }
      if (d.content && d.content.includes('Total Bytes:')) {
        console.log(`Step ${d.step_index}: ${d.content.match(/Total Bytes:\s*([0-9]+)/)?.[1]}`);
      }
    }
  } catch(e) {}
});
